...
public class BarResController {
    ...
    public ResponseEntity<Page<BarRes>> getAllBarRes(
        ...
        if (barResType != null) {
            Categories categories = new BarRes().getTypeByName(barResType);
            barResPage = barResService.findByBarResType(categories, pageable);
        } else {
            barResPage = barResService.findAll(pageable);
        }

        return ResponseEntity.ok(barResPage);
    }
    ...
    public ResponseEntity<?> update (@RequestBody BarRes barResDetails, @PathVariable UUID id) {
        ...
        barRes.get().setBarResFotoId(barResDetails.getBarResFotoId());
